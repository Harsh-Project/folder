...
const moduleOverride = {
  name: 'storeFrontSharedApi',
  filename: `assets/store_front_shared_api.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.js`,
  exposes: {
    "./storeFrontSharedApi": "./src/storefront-shared-apis/allAPI.jsx",
  },
  remotes: {
  },
  shared: {
...
const configOutputOverRide = {
  chunkFilename: `assets/[name].[contenthash:8].storeFrontSharedApi.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.chunk.js`,
  filename: `assets/[name].[contenthash:8].storeFrontSharedApi.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.js`,
  // chunkFormat: "commonjs",
  publicPath: "auto",
  path: path.resolve(__dirname, "build")
};
const cssPaths = {
  filename: `assets/[name].[contenthash:8].storeFrontSharedApi.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.css`,
  chunkFilename: `assets/[name].[contenthash:8].storeFrontSharedApi.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.chunk.css`,
}
