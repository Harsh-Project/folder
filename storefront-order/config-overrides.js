...
const moduleOverride = {
  ...
  filename: `assets/store_front_order.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.js`,
  exposes: {
    "./storeFrontOrder": "./src/App.js"
  },
  remotes: {
  },
  shared: {
...
const configOutputOverRide = {
  chunkFilename: `assets/[name].[contenthash:8].storeFrontOrder.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.chunk.js`,
  filename: `assets/[name].[contenthash:8].storeFrontOrder.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.js`,
  // chunkFormat: "commonjs",
  publicPath: "auto",
  path: path.resolve(__dirname, "build")
};
const cssPaths = {
  filename: `assets/[name].[contenthash:8].storeFrontOrder.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.css`,
  chunkFilename: `assets/[name].[contenthash:8].storeFrontOrder.${process.env.REACT_APP_CODE_BUILD_VERSION ?? ""}.chunk.css`,
}
