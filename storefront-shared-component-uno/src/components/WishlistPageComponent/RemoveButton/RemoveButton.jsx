const styles = await import("./RemoveButton.module.css").then(
...
export const RemoveButton = ({ value, show, handleRemoveClick }) => {
  ...
  const handleRemoveFromList = async () => {
    handleRemoveClick(value, wishlistKeyInLocalStorage);
  };
  return (
    <>
      <button
        ...
        className={`${styles.flits_button} ${styles.flits_primary_btn} ${
          styles.flits_mt_10
        } ${styles.flits_remove_product} ${show ? styles.flits_set_show : ""}`}
      >
        X
      </button>
    </>
