  ...
  useEffect(() => {
    if (buttonRef.current) {
      tippy(buttonRef.current, {
        content: wishListed
          ? t(
              "flits.wishlisted_product_page.remove_from_wishlist_button",
              "Added to Wishlist"
            )
          : t(
    ...
    >
      <div className={styles.flits_wishlist_button}>
        ...
        >
          ...
          {parseInt(
            window?.flitsThemeAppExtensionObjects?.Metafields?.WSL_BTN_TYPE
          ) === 1 && (
            <div
        ...
        {parseInt(
          ...
          <div className={styles.flits_wishlist_count}>{wishListCount}</div>
        ) : null}
      </div>
    </div>
  );
