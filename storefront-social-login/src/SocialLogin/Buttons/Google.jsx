...
export const Google = () => {
  ...
  const { t } = getStore._globalActions.Helpers[0].useTranslationLanguage();
    return (
        <a
        style={{ order: 2 }}
        onClick={() => SocialLoginButtonClick("google",{referalRegisterPage})}
        className={`${styles.flits_social_login_btn} ${styles.flits_social_login_btn_googleplus}`}
      >
        <span className={styles.flits_social_login_btn_img}>
          <svg
            className={styles.flits_icon_google}
            version="1.1"
            id="Layer_1"
            x="0px"
            y="0px"
        ...
        <span className={styles.flits_social_login_btn_text}>{t("flits.social_login_page.google", "Google")}</span>
      </a>
    );
}