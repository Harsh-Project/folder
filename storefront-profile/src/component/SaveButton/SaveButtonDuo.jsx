...
const profile = (
  <svg id="Layer_1" data-name="Layer 1" viewBox="0 0 40.01 40.01">
    <path
      id="Layer_2"
      data-name="Layer 2"
      d="M39.47,4.19a1.81,1.81,0,0,1,0,2.56h0L21.29,24.94h0a1.82,1.82,0,0,1-2.57,0l-7.27-7.28A1.82,1.82,0,0,1,14,15.09l6,6L36.89,4.18h0a1.82,1.82,0,0,1,2.57,0ZM39.6,16a1.82,1.82,0,1,0-3.57.72,16.34,16.34,0,1,1-8-11,1.82,1.82,0,0,0,1.78-3.18A20,20,0,1,0,40,20,20.71,20.71,0,0,0,39.6,16Z"
      transform="translate(0 0.03)"
    ></path>
  </svg>
);

export const SaveButtonDuo = ({ item }) => {
  ...
  const MyProfileSaveButton =
    window.UnoDuoComponent("MyProfileSaveButton");
  ...
  const setFirstNameInitial =
    window.profileState("setFirstNameInitial");
  const setLastNameInitial =
    window.profileState("setLastNameInitial");
  const setContact = window.profileState("setContact");
  const setBirthdateInitial =
    window.profileState("setBirthdateInitial");
  const setGenderInitial =
    window.profileState("setGenderInitial");
  const setContactInitial =
    window.profileState("setContactInitial");
  ...
  const handleClickChange = async () => {
    ...
    if (
      ...
      birthdate?.day !== "" &&
      birthdate?.month !== "" &&
      birthdate?.year !== "" && birthdate?.day !=="DD" && birthdate?.month !== "MM" && birthdate?.year !== "YYYY"
    ) {
      formData.birthdate = `${birthdate?.year}-${birthdate?.month}-${birthdate?.day}`;
    }
    ...
    if (
      ...
      (birthdate?.day === "" ||
        birthdate?.month === "" ||
        birthdate?.year === "")) || (typeof birthdate === "object" &&
        (birthdate?.day === "DD" ||
          birthdate?.month === "MM" ||
          birthdate?.year === "YYYY"))
    ) {
    ...
    setSnackbarMessage(
      t("flits.profile_page.save_details", "Saving profile details...")
    );
    ...
    if (profile_save?.status) {
      setSnackbarMessage(
        t(
          "flits.profile_page.saved_successfully",
  ...
  if (!MyProfileSaveButton || !MyProfileSnackBar) {
    return null;
  }
  ...
  return (
    <>
      ...
      <SnackBar mode={openSnackbar} message={snackbarMessage} svg={profile} />
    </>
  );
};
