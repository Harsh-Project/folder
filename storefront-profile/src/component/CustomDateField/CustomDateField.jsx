...
export const CustomDateField = ({ item }) => {
  ...
  const MyProfileCustomDateField =
    window.UnoDuoComponent("MyProfileCustomDateField");
  ...
  const IsRequiredDateValidationFailed =
    getStore._globalActions.Helpers[0].IsRequiredDateValidationFailed;

  const validateField = useCallback(
    (value) => {
      let isErrorInField = false;
      let failedValidationRule = null;
      if (item?.is_required && IsRequiredDateValidationFailed(value)) {
        isErrorInField = true;
    ...
    [IsRequiredDateValidationFailed, item]
  );
  ...
  if (!MyProfileLabel || !MyProfileCustomDateField) return null;
  ...
  return (
    <MyProfileCustomRow edit={edit} index={item?.unique_id}>
      ...
      <MyProfileCustomDateField
        ...
        placeholder={edit && item?.placeholder ? item?.placeholder : ""}
        edit={edit}
      />
    </MyProfileCustomRow>
  );
