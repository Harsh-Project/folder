import styles from "./SelectTopOrderModule.module.css";
        ...
        {item?.map((data, index) => (
          <option {...data} key={index}>
            {(data?.available === "true" || data?.available === true)
              ? data?.product_title
              : `${data?.product_title?.replace(" - Sold Out", "")} - ${t("flits.buttons.sold_out",
              "Sold Out")}`}
          </option>
        ))}
      </select>
      <div className={styles.flits_select_arrow}><RenderSvgString svgString={window?.DuoIcon?.DownArrow} /></div>
    </div>
  );
};
