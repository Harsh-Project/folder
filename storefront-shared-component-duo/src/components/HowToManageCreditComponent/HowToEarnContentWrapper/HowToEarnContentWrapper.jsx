import styles from "./HowToEarnContentWrapperModule.module.css";
import React, { Suspense } from "react";
import { useSelector } from "react-redux";
import { GlobalStore } from "redux-micro-frontend";
import { RenderSvgString } from "../../General/RenderSvgString";
import { HowToEarnHeaderImage } from "../HowToEarnHeader/HowToEarnHeaderImage/HowToEarnHeaderImage"
import { HowToEarnHeaderChart } from "../HowToEarnHeader/HowToEarnHeaderChart/HowToEarnHeaderChart"
import { UserBox } from "../../General/UserBox/UserBox"

export const HowToEarnContentWrapper = (props) => {
  const firstNameInitial = useSelector(
    (state) => state.storeFrontMyProfile.firstNameInitial
  );
  const lastNameInitial = useSelector(
    (state) => state.storeFrontMyProfile.lastNameInitial
  );

  const getStore = GlobalStore.Get();

  const { t } = getStore._globalActions.Helpers[0].useTranslationLanguage();
  return (
    <>
      <div
        className={`${styles.flits_account_box} ${styles.flits_how_to_earn_div}`}
      >
        <div className={styles.flits_how_to_earn_container}>
          {props?.children}
        </div>
      </div>
      <div
        className={`${styles.flits_mobile_account_box} ${styles.flits_how_to_earn_div}`}
      >
        <div className={styles.flits_how_to_earn_container}>
          <div
            className={`${styles.flits_navigation_header} ${styles.flits_with_box_shadow} ${styles.flits_pb_30}`}
          >
            <a
              href={window?.commonEndpoint?.logout ?? "/account/logout"}
              className={styles.flits_logout_button}
              noInstant=""
              data-no-instant=""
            >
              <RenderSvgString svgString={window?.DuoIcon?.LogoutMobile} />
            </a>
            <div
              className={styles.flits_user_avatar}
              data-flits-name={`${
                firstNameInitial?.length > 0
                  ? firstNameInitial.charAt(0).toUpperCase()
                  : ""
              }${
                lastNameInitial?.length > 0
                  ? lastNameInitial.charAt(0).toUpperCase()
                  : ""
              }`}
            ></div>
            <div className={styles.flits_navigation_header_content}>
              <Suspense fallback={<></>}>
                <HowToEarnHeaderChart />
                <UserBox />
                <HowToEarnHeaderImage />
              </Suspense>
            </div>
          </div>
          <div
            className={`${styles.flits_mobile_box_card} ${styles.flits_header_title} ${styles.flits_p_10}`}
          >
            <span
              dangerouslySetInnerHTML={{
                __html: t(
                  "flits.how_to_earn_credit_page.header_line_html",
                  "Earn More <strong>Spend More</strong>"
                ),
              }}
            ></span>
          </div>
          {props?.children}
        </div>
      </div>
    </>
  );
};
